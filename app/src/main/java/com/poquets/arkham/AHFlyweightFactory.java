...
public class AHFlyweightFactory {
	...
	public ArrayList<NeighborhoodCard> getCurrentNeighborhoodsCards(long neiID)
	{
		...
		Log.i("AHFlyweightFactory", "getCurrentNeighborhoodsCards - neiID: " + neiID + ", expIDs: '" + expIDs + "'");
		
		// If no expansions selected, default to base game (expansion ID 1)
		if (expIDs.isEmpty()) {
			expIDs = "1";
			Log.i("AHFlyweightFactory", "No expansions selected, using base game (expID 1)");
		}
		...
		Log.i("AHFlyweightFactory", "Returning " + cards.size() + " cards for neighborhood " + neiID);
		return cards;
	}
